...
////////////////////////////////////////////////////////////////////////////////
// misc
static int _PyString_Check(PyObject *o) { return PyString_Check(o); }
...
PyObject *WSGI_APPLICATION  = NULL;
...
static PyObject *Adapter_start_response(AdapterObject *self, PyObject *args)
{
    ...
    self->status = (int)strtol(PyString_AsString(status_line), NULL, 10);
...
static Response run_wsgi_application(char *body, PyObject* environ) {
	...
	if (WSGI_APPLICATION == NULL) {
		...
		WSGI_APPLICATION = PyObject_GetAttr(_module, PyString_FromString("application"));
	}
    ...
    PyDict_SetItemString(environ, "wsgi.version", WSGI_VERSION);

    PyObject* _body = PyObject_CallObject(
		WSGI_APPLICATION,
		Py_BuildValue(
			"(OO)",
			environ,
			PyObject_GetAttrString((PyObject *)_adapter, "start_response")
		)
	);
	...
	response.body = _body;
	response.error = _adapter->wsgi_error;
...
func wsgi_callback(r *http.Request, w http.ResponseWriter) {
	...
	defer func() {
		C.Py_DecRef(_response.body)
		C.Py_DecRef(_response.headers)
		C.Py_DecRef(_response.error)
	}()
	...
	write_response_body(w, _response.body)
}
